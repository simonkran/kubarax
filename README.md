...
1. **A Kubernetes cluster** with a `kubectl` context configured and pointing to it
2. **A Git repository** (HTTPS) that Flux will sync from — this is where kubarax writes its generated manifests
3. **A DNS zone** (e.g. Cloudflare, AWS Route53) with a domain you control for platform ingress
4. **Secrets in your secret manager** — if you use the External Secrets Operator, pre-create the required secrets (API tokens, OAuth credentials, etc.) in your backend (1Password, Vault, AWS Secrets Manager, ...) before the platform services can reconcile. See the [full secrets reference](docs/add-external-secrets.md#secrets-to-pre-create-in-your-secret-manager) for a per-service breakdown
5. **Bootstrap credentials** in a `.env` file — Git PAT, domain name, and optional Docker/Helm registry credentials (generated by `kubarax init --prep`)
